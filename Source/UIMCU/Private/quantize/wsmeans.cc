...
QuantizerResult QuantizeWsmeans(const std::vector<Argb>& input_pixels,
  ...
  TMap<Argb, int> pixel_to_count;
  std::vector<uint32_t> pixels;
  pixels.reserve(pixel_count);
  std::vector<Lab> points;
  points.reserve(pixel_count);
  for (Argb pixel : input_pixels) {
      // Look for the pixel in the map
      if (pixel_to_count.Contains(pixel)) {
          // If it exists, increment the count
          pixel_to_count[pixel]++;
      }
      else {
          // Otherwise, add the pixel to the array and map
          pixels.push_back(pixel);
          points.push_back(LabFromInt(pixel));
          pixel_to_count.Add(pixel, 1);  // Set the initial count to 1
      }
  }
